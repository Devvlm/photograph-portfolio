/**
 * Translations
 * GET  /api/pricing-text/:lang   — pricing overrides from KV (public)
 * PUT  /api/pricing-text/:lang   — save pricing overrides (requires auth)
 * GET  /api/translations/:lang   — all other section overrides from KV (public)
 * PUT  /api/translations/:lang   — save all section overrides (requires auth)
 */
...
export async function handleAllTranslations(request, env, { jsonResponse, errorResponse }) {
  const url = new URL(request.url);
  const parts = url.pathname.split('/').filter(Boolean);
  const lang = parts[parts.length - 1];
  ...
  if (request.method === 'GET') {
    if (!env.TRANSLATIONS_KV) return jsonResponse({}, 200, env, request);
    const stored = await env.TRANSLATIONS_KV.get(kvKey);
    if (!stored) return jsonResponse({}, 200, env, request);
    return jsonResponse(JSON.parse(stored), 200, env, request);
  }
  ...
  if (request.method === 'PUT') {
    ...
    if (!env.TRANSLATIONS_KV) return errorResponse('KV storage not configured', 503, env, request);

    let body;
    try { body = await request.json(); }
    catch { return errorResponse('Invalid JSON body', 400, env, request); }
